...
<script lang="ts">
    ...
    onMount(async () => {
        ...
        const fcRoot = (await import('svelte-fusioncharts')).fcRoot
        SvelteFC = (await import('svelte-fusioncharts')).default
...
<div>
    {#if chartConfigs && actions.length > 0 && closePrices.length > 0 && volumes.length > 0}
        <p>before</p>
        <svelte:component this={SvelteFC} {...chartConfigs}/>
        <p>after</p>
    {/if}
    <p>end</p>
</div>
